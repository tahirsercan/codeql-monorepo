...
/**
* lastRSS
* Simple yet powerfull PHP class to read and parse RSS files.
* @version 1.1
*/
class LastrssPlugin extends BasePluginClass 
{
	// -------------------------------------------------------------------
	// Public properties
	// -------------------------------------------------------------------
	public $cache_dir="/tmp"; //empty string to disable
	...
	public function Get ($rss_url) {
		...
		if ($this->cache_dir != '') {
			...
			if (file_exists($cache_file)) 
				...
				$timedif=$this->cache_time+1; //force get
 ...
 * 	$r = new LastrssPlugin ( j::$App );
	...
	foreach ( $rss ['items'] as $i )
	{
		...
		if ($n>=5) return;
		$x = $i ['description'];
		//	$x=substr($x,0,255).((strlen($x)>255)?"...":"");
		echo "<h3><a href='{$i['link']}'>{$i['title']}</a></h3><p>{$x}</p>";
	}
